...
module.exports.handleReply = async function({
  ...
  switch (handleReply.type) {
    ...
    case "exp": {
      ...
      if (content > exp)
        api.sendMessage(
          "Exp của bạn không đủ ? vui lòng cào phím nhiều hơn",
          event.threadID,
          event.messageID
        );
      else {
    ...
    case "money": {
      ...
      if (content > money)
        api.sendMessage(
          "Tiền của bạn không đủ ? vui lòng theo thầy Huấn bươm trải !",
          event.threadID,
          event.messageID
        );
      else {
        ...
        var exp =
          ...
          parseInt(content / 10);
        await Currencies.setData(handleReply.author, { exp });
        var msg = `💸Giao dịch thành công !\nThời gian: ${time} - ${date}\nChi tiết: đổi ${content} đô để lấy ${content /
          10} exp.`;
        api.sendMessage(msg, handleReply.author);
        const suggest = msg;
        getData.push(suggest);
